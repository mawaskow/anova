...
def instructions(w,x,y):
    ...
    line2 =Text(Point(x/2, 200+30), "Four Windows Dialog boxes for 3 input files and 1 output file appear requsting the names of 4 files (2 are the same):").draw(w)
...
def anovadisp(a,b,c,d,e,f,g,h,i,j,k,l,m,n,o,p,q,r,w,x,y):
    ...
    outfile.close()
    # now write to screen
    okBox(w,x,y,"***Select the output.txt file that the results were written to***", "dark blue", "pink")
    otpt= open(askopenfilename(), "r")
    filotpt = [row for row in csv.reader((otpt))]
    okBox(w,x,y,"Click here to write results to screen", "dark blue", "pink")
    line3=Text(Point(x/2,170),"ANOVA Program Output").draw(w)
    line3.setSize(16), line3.setTextColor('Dark Blue')
    for i in range(len(filotpt)):
        messy = Text(Point(x/2,200+(30*i)), filotpt[i]).draw(w)
    okBox(w,x,y,"Click here to format output in a table", "dark blue", "pink")
    w.close()
    # make data nice
    v = GraphWin("ANOVA Results Table", x, y)
    beLogo(v,x,y)
    line3=Text(Point(x/2,170),"ANOVA Program Output").draw(v)
    line3.setSize(16), line3.setTextColor('Dark Blue')
    line4=Text(Point(x/2,200+30),(a + (" "*5) + b + (" "*5) + c + (" "*5) + d + (" "*5) + e + (" "*5))).draw(v)
    line5=Text(Point(x/2,200+60),("-" * 110)).draw(v)
    line6=Text(Point(x/2,200+90),(str(f) + (" "*10) + str(g) + (" "*30) + str(h) + (" "*30) + str(i) + (" "*13) + str(j) + (" "*22))).draw(v)
    line7=Text(Point(x/2,200+120),(str(k) + (" "*10) + str(l) + (" "*30) + str(m) + (" "*30) + str(round(n, 1)) + (" "*45))).draw(v)
    line8=Text(Point(x/2,200+150),("-" * 110)).draw(v)
    line9=Text(Point(x/2,200+180),(str(o) + (" "*13) + str(p) + (" "*28) + str(q)+ (" "*64))).draw(v)
    line10=Text(Point(x/2,200+210),("The value for F at alpha = 0.05 is: " + str(r) + (" "*70))).draw(v)
    if (float(j) > float(r)):
        line11 = Text(Point(x/2,200+240),("There IS a significant statistical difference between the datasets." + (" "*25))).draw(v)
    else:
        line11 =Text(Point(x/2,200+240),("There is NOT a significant statistical difference between the datasets." + (" "*25))).draw(v)
    okBox(v,x,y, "Click here to exit", "Dark Blue", "Yellow")
    return v
...
def main():
    ...
    okBox(w,x,y, "Please Click Here to Continue", "dark blue", "yellow")
...
###
    ...
    v = anovadisp("Source","Sum of Squares","Degrees of Freedom","Mean Square","F Test","Between Samples", ssb, degfb, meansqb, ftst, "Within Samples", ssw, degfw, meansqw, "Total", tss, degft, alpha, w, x, y)
###
    #vgetMouse()
    v.close()
